...
export function RoadmapDisplay({ roadmap }: { roadmap: Roadmap }) {
  ...
  return (
    <div className="mt-8 bg-gray-50 p-6 rounded-lg border border-gray-200 relative">
      ...
      <h3 className="text-2xl font-semibold text-blue-800 mb-4">Your MVP Roadmap</h3>
      {roadmap.phases.map((phase, index) => (
        <div key={index} className="mb-6">
          <div className="flex justify-between items-start mb-3">
            <h4 className="text-xl font-medium text-blue-700">{phase.name}</h4>
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">{phase.timeline}</span>
          </div>
            ...
            <div>
              ...
              <ul className="list-disc ml-6 space-y-1">
                {phase.tasks?.map((task, taskIndex) => (
                  <li key={taskIndex} className="text-gray-800">{task}</li>
                ))}
              </ul>
            </div>
...
export function FeatureList({ formik }: { formik: any }) {
  const addFeature = () => {
    ...
    formik.setFieldValue("features", [...formik.values.features, formik.values.newFeature.trim()]);
  ...
  return (
    <div>
      <label className="block text-gray-700 font-medium mb-2">Core Features (3-5)</label>
...
export function RoadmapForm({
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-6">
      <div>
        ...
        <input
          ...
          className={`w-full p-3 border ${
            formik.errors.appName && formik.touched.appName ? "border-red-600" : "border-gray-300"
      ...
      <div>
        <label className="block text-gray-700 font-medium mb-2">App Purpose</label>
        <textarea
          name="purpose"
          placeholder="Describe your app's main purpose and target audience..."
          className={`w-full p-3 border ${
            formik.errors.purpose && formik.touched.purpose ? "border-red-600" : "border-gray-300"
